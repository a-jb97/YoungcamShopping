...
final class SearchResultViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        viewModel.inputProductName.value = searchBarText ?? ""
        bindData(sort: Sorted.sim.rawValue)
    ...
    func bindData(sort: Sorted.RawValue) {
        viewModel.inputProductName.bind {
            let url = "https://openapi.naver.com/v1/search/shop.json?query=\(self.viewModel.inputProductName.value)&display=100=10&start=\(self.start)&sort=\(sort)"
            ...
            AF.request(url, method: .get, headers: headers).validate(statusCode: 200..<300).responseDecodable(of: ProductInfo.self) { response in
                ...
                case .success(let value):
                    ...
                    self.navigationItem.title = self.viewModel.inputProductName.value
    ...
    @objc private func dateOrderButtonTapped() {
        bindData(sort: Sorted.date.rawValue)
        self.productCollectionView.reloadData()
    }
    
    @objc private func highPriceButtonTapped() {
        bindData(sort: Sorted.dsc.rawValue)
        self.productCollectionView.reloadData()
    }
    
    @objc private func lowPriceButtonTapped() {
        bindData(sort: Sorted.asc.rawValue)
        self.productCollectionView.reloadData()
    }
